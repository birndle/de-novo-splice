...
get_proximal_feats <- function(slice, k=6) {
  count_inner <- function(cell, intronic) {
    if (is.na(cell)) {
      ...
      if (intronic) {
        return(sum((loc > slice$RefLoc + 6) & (loc + k < slice$AltLoc - 3)))
      } else {
        return(sum((loc > slice$AltLoc + 6) & (loc + k < slice$RefLoc - 3)))
      }
    }
  }
...
get_distal_feats <- function(slice, k=6) {
  count_outer <- function(cell, intronic) {
    if (is.na(cell)) {
      return(0)
    } else {
      ialt = min(slice$IntronEnd - 20, slice$AltLoc + 100)
      ealt = max(slice$ExonStart + 3, slice$AltLoc - 100)
      loc = strsplit(as.character(cell), ",")[[1]] %>% as.numeric
      if (intronic) {
        return(sum((loc > slice$AltLoc + 6) & (loc + k < ialt)))
      } else {
        return(sum((loc > ealt) & (loc + k < slice$AltLoc - 3)))
      }
    }
  }
  feats = slice[,!(colnames(slice) %in% other)]
...
get_exonic_feats <- function(slice, k=6) {
  count_exonic <- function(cell, intronic) {
    if (is.na(cell)) {
      return(0)
    } else {
      eref = max(slice$ExonStart + 3, slice$RefLoc - 100)
      loc = strsplit(as.character(cell), ",")[[1]] %>% as.numeric
      return(sum((loc > eref) & (loc + k < slice$RefLoc - 3)))
    }
  }
  feats = slice[,!(colnames(slice) %in% other)]
...
get_intronic_feats <- function(slice, k=6) {
  count_intronic <- function(cell) {
    if (is.na(cell)) {
      return(0)
    } else {
      iref = min(slice$IntronEnd - 20, slice$RefLoc + 100)
      loc = strsplit(as.character(cell), ",")[[1]] %>% as.numeric
      return(sum((loc > slice$RefLoc + 6) & (loc + k < iref)))
    }
  }
  feats = slice[,!(colnames(slice) %in% other)]
...
# decoys
# ese_raw_decoy = derive_feats_df(read.table(sprintf('decoy/data/%s/raw/5pr_ese_feats.txt',vers), header=T))
...
# decoy
# ess_raw_decoy = derive_feats_df(read.table(sprintf('decoy/data/%s/raw/5pr_ess_feats.txt',vers), header=T))
...
gtrip_novo = list()
gtrip_novo$proximal = adply(gtrip_raw_novo, 1, get_proximal_feats, 3)
gtrip_novo$distal = adply(gtrip_raw_novo, 1, get_distal_feats, 3)
gtrip_novo$exonic = adply(gtrip_raw_novo, 1, get_exonic_feats, 3)
gtrip_novo$intronic = adply(gtrip_raw_novo, 1, get_intronic_feats, 3)
# cryptic
gtrip_raw_cryp = derive_feats_df(read.table('../cryptic/data/raw/cryptic5_gtrip_feats.txt', header=T, sep="\t"))
gtrip_cryp = list()
gtrip_cryp$proximal = adply(gtrip_raw_cryp, 1, get_proximal_feats, 3)
gtrip_cryp$distal = adply(gtrip_raw_cryp, 1, get_distal_feats, 3)
gtrip_cryp$exonic = adply(gtrip_raw_cryp, 1, get_exonic_feats, 3)
gtrip_cryp$intronic = adply(gtrip_raw_cryp, 1, get_intronic_feats, 3)
# decoy
# gtrip_raw_decoy = derive_feats_df(read.table(sprintf('decoy/data/%s/raw/5pr_gtrip_feats.txt', vers), header=T, sep="\t"))
# gtrip_raw_decoy = gtrip_raw_decoy[use_idx,]
# gtrip_decoy = list()
# gtrip_decoy$proximal = adply(gtrip_raw_decoy, 1, get_proximal_feats, 3)
# gtrip_decoy$distal = adply(gtrip_raw_decoy, 1, get_distal_feats, 3)
# gtrip_decoy$exonic = adply(gtrip_raw_decoy, 1, get_exonic_feats, 3)
# gtrip_decoy$intronic = adply(gtrip_raw_decoy, 1, get_intronic_feats, 3)
